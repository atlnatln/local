...
#   2. Copilot CLI (claude-haiku-4.5) AGENTS-LEVELS.md ile yeni 12 seviye üretir
...
if [ -f "$LEVELS_FILE" ]; then
    ...
    AGE_GROUP=$(python3 -c "import json; d=json.load(open('$LEVELS_FILE')); print(d.get('ageGroup','7-8'))" 2>/dev/null || echo "7-8")
fi
...
if [ "$DRY_RUN" = true ]; then
    echo -e "\n${YELLOW}--dry-run: Copilot çalıştırılmadı.${NC}"
    exit 0
fi
...
# ─── 3. Copilot ile üret ───────────────────────────────────
echo -e "\n${YELLOW}[3/5] 🤖 Copilot CLI ile 12 yeni seviye üretiliyor...${NC}"

# AGENTS.md swap — copilot CLI levels agent'ı okusun
agents_swap_in "agents/levels.agents.md"

PROMPT="AGENTS.md dosyasındaki tüm kuralları uygula.

ADIMLAR:
1. data/levels.json oku — mevcut version numarasını al
2. data/level-stats.json oku (varsa) — oyuncunun performansını analiz et
3. Yeni 12 seviye üret → data/levels.json'a yaz (üzerine yaz)
...
KRİTİK KURALLAR:
- Sadece data/levels.json değiştir, başka dosyaya dokunma
