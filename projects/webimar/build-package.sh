...
# Do NOT fail if secrets file is absent; prefer example.
if [ -f ".env.production" ]; then
    # NOTE: This may include secrets. Use with care.
    cp .env.production "${BUILD_DIR}/.env"
    echo -e "${GREEN}  ✓ Included .env from .env.production${NC}"
elif [ -f ".env.production.example" ]; then
    cp .env.production.example "${BUILD_DIR}/.env.example"
    echo -e "${YELLOW}  ⚠️  .env.production not found; packaged .env.production.example as .env.example${NC}"
else
    echo -e "${RED}❌ Missing both .env.production and .env.production.example${NC}"
    exit 1
fi
