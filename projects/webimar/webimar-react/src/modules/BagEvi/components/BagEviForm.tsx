...
const BagEviForm: React.FC<BagEviFormProps> = ({
  formData,
  validationErrors,
  onInputChange,
  ...
  return (
    <FormGrid>
      ...
      {(formData.arazi_vasfi === 'Dikili vasıflı' || 
        ...
        <FormField
          label="Dikili Alanı (m²)"
          name={formData.arazi_vasfi === 'Dikili vasıflı' ? 'alan_m2' : 'dikili_alani'}
          type="number"
          value={formData.arazi_vasfi === 'Dikili vasıflı' ? (formData.alan_m2 || '') : (formData.dikili_alani || '')}
          onChange={onInputChange}
          ...
          step="1"
          required
          error={formData.arazi_vasfi === 'Dikili vasıflı' ? validationErrors.alan_m2 : validationErrors.dikili_alani}
          {...commonFieldProps}
        >
          {renderSmartDetectionFeedback('dikili_alani')}
        </FormField>
      ...
      {formData.arazi_vasfi === 'Zeytin ağaçlı + tarla' && (
        <>
          ...
          <FormField
            label="Zeytin Ağacı Sayısı (adet)"
            name="zeytin_agac_adedi"
            type="number"
            value={formData.zeytin_agac_adedi || ''}
            onChange={onInputChange}
            placeholder="Örn: 150"
            min="0"
            step="1"
            required
            error={validationErrors.zeytin_agac_adedi}
            helpText="Zeytin ağacı sayınızı girin. Hesaplama sonucu pozitif veya negatif çıkabilir."
            {...commonFieldProps}
          />
        </>
