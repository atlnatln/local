// Validation utilities - simplified for issue #10
// Validation utilities - simplified for numeric password rule
...
// Şifre validasyonu (sadece rakam ve min. 4 karakter)
export const validatePassword = (password: string): ValidationResult => {
  ...
  if (!/^\d+$/.test(password)) {
    return {
      isValid: false,
      error: 'Şifre sadece rakamlardan oluşmalı.'
    };
  }
  return {
    isValid: true
