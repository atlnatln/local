...
while [ $ATTEMPT -lt $MAX_RETRIES ] && [ "$SUCCESS" = false ]; do
    ...
    if command -v copilot &>/dev/null; then
        copilot -p "$PROMPT" --model gpt-4.1 --yolo 2>&1
    else
        gh copilot -p "$PROMPT" --model gpt-4.1 --yolo 2>&1
    fi
...
if [ "$SKIP_SYNC" = false ]; then
    ...
    # Sağlık kontrolü
    sleep 1
    HEALTH=$(ssh "$VPS_HOST" "curl -sf -H 'Host: 89.252.152.222' http://localhost/mathlock/data/questions.json 2>/dev/null | python3 -c 'import json,sys; d=json.load(sys.stdin); print(f\"v{d[\\\"version\\\"]} — {len(d[\\\"questions\\\"])} soru\")' 2>/dev/null || echo 'FAIL'")
    if [ "$HEALTH" != "FAIL" ]; then
