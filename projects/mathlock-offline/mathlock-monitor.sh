#!/bin/bash
# ============================================================================
# MathLock Stats Monitor — Otomatik AI Soru Üretim Tetikleyicisi
#
...
LOCK_FILE="/tmp/mathlock-ai-generate.lock"
SERVICE_NAME="mathlock-monitor"
...
case "${1:-}" in
    --install)
        echo -e "${CYAN}[KURULUM]${NC} mathlock-monitor systemd user service kuruluyor..."
        mkdir -p "$HOME/.config/systemd/user"
        cat > "$HOME/.config/systemd/user/${SERVICE_NAME}.service" << EOF
[Unit]
Description=MathLock Stats Monitor — Otomatik AI Soru Üretimi
...
log "=== MathLock Monitor başlatıldı ==="
...
try:
    ...
    # Sadece test verisi değil, gerçek bir set tamamlanmış mı?
