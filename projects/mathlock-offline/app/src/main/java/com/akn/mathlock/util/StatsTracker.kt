...
/**
 * Offline mod: sonuçları lokal kaydeder, sunucuya yüklemez.
 */
class StatsTracker(private val context: Context) {
    ...
    /**
     * Offline mod: stats sadece lokal temizlenir, sunucuya yüklenmez.
     */
    fun uploadStats(questionVersion: Int): Boolean {
        results.clear()
        prefs.edit().remove(KEY_RESULTS).apply()
        Log.d(TAG, "Offline mod: stats temizlendi (v$questionVersion)")
        return true
    }
