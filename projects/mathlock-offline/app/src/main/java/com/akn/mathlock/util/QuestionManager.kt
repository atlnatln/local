...
/**
 * Offline mod: assets'teki questions.json'dan soruları yükler.
 * Set tamamlanınca otomatik sıfırlanır (sonsuz döngü).
 */
class QuestionManager(private val context: Context) {
    ...
    /**
     * Offline mod: assets'ten questions.json yükle.
    ...
    private fun loadFromAssets(): Boolean {
        return try {
            ...
            if (parseQuestions(json)) {
                ...
                _isJsonMode = true
                Log.d(TAG, "Assets'ten yüklendi: v$version, ${questions.size} soru")
                true
            } else false
        } catch (e: Exception) {
