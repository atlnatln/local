...
- `generate(educationPeriod)` çağrısıyla çalışır
- Her yaş grubu için aynı sayı aralıklarını kullanır ama deterministic DEĞİLDİR (`Random.nextInt`)
- JSON cache yoksa `startFallbackMode()` ile bu generator çalışır
- `generateGrade1()`: toplama/çıkarma (1-10 arası), sonuç max 20. Çıkarmada onluktan bozma kontrolü var (`b <= a % 10`)
- `generateGrade2()`: toplama(50,50), çıkarma(10,99), çarpma(10,10), bölme(10,10)
- `generateGrade3()`: toplama(100,100), çıkarma(10,200), çarpma(12,12), bölme(12,12)
- `generateGrade4()`: toplama(500,500), çıkarma(50,500), çarpma(15,15), bölme(15,15)

**Hint ekleme (`HINTS` sözlüğü):**
- Her soruya `type`'a göre rastgele hint seçilir
